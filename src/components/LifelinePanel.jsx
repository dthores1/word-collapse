...
//   compact   — horizontal row used in the mobile control strip below
//               the board. Sized at 56×56 so the touch target is
//               generous — these are *gameplay abilities* (clear a row,
//               compact the stack), not utility controls.
//
...
export function LifelinePanel({
  ...
  const wrapper = compact ? 'flex gap-3' : 'flex gap-3 self-center flex-col';
  return (
    <div className={wrapper}>
      ...
      />
      <LifelineButton
        icon={<ShuffleIcon compact={compact} />}
        label="Scramble"
        sublabel="Reshuffle tiles"
        uses={scrambleUses}
        onClick={onScramble}
        tone="warn"
        compact={compact}
...
function LifelineButton({ icon, label, sublabel, uses, onClick, tone, compact }) {
  ...
  const enabledColor =
    tone === 'danger'
      ...
      : tone === 'warn'
        ? 'bg-paper border-warn-500/40 hover:border-warn-500 hover:shadow-lg active:scale-95 text-warn-500'
  ...
  const iconWrapClass = compact ? 'w-6 h-6' : 'w-8 h-8';
...
// Lucide's Shuffle glyph — two crossing arrows that swap rails.
function ShuffleIcon({ compact }) {
  const size = compact ? 24 : 32;
  return (
    <svg
      width={size}
